...
//              só o `screenshot-path` (Config.screenshotDir, editável em Gravação) —
//              a linha equivalente no config.kdl deve ficar comentada p/ não competir.
//              As cores de focus-ring/border NÃO vêm mais por include: o config.kdl é
//              editado DIRETO (com backup .bak-$ts, como os demais alvos), reescrevendo
//              só as 4 linhas active/inactive-color de dentro do bloco `layout` — é o
//              bloco que o NiriMod (github.com/srinivasr/NiriMod, GUI externa de config
//              do niri que o usuário passou a usar) escreve lá, e por vir DEPOIS do
//              include no arquivo, sempre venceria o merge do niri. Ver
//              niriLayoutPatchPy()
...
Singleton {
    ...
    // ── niri (devil-shell/theme.kdl) — só o screenshot-path ──
    // As cores de focus-ring/border NÃO vêm mais por aqui. Desde que o usuário passou a
    // editar o layout do niri pelo NiriMod (GUI externa,
    // https://github.com/srinivasr/NiriMod), ela escreve seu PRÓPRIO bloco `layout`
    // direto no config.kdl (depois do `include` desta linha) — e como o niri, ao
    // mergear blocos `layout` duplicados, deixa o que vem DEPOIS no arquivo vencer pra
    // cada propriedade escalar, o bloco do NiriMod sempre sobrepunha o nosso: colorir
    // só o include tinha parado de ter efeito. Ver niriLayoutPatchPy()/exportAll(), que
    // edita essas 4 linhas DIRETO dentro do bloco `layout` do config.kdl (o que o
    // NiriMod escreve), preservando o resto (gaps/shadow/width/formatação dele).
    function niriContent() {
        return "// Auto-generated by quickshell. Do not edit by hand.\n"
            ...
            + "screenshot-path \"" + Config.screenshotDir + "/Screenshot from %Y-%m-%d %H-%M-%S.png\"\n"
    }

    // ── niri: patch direto do config.kdl — cores do bloco `layout` do NiriMod ──
    // Devolve um script Python (gravado num temp e rodado via `python3 <temp> <config.kdl>`
    // por exportAll()) que reescreve só as linhas active-color/inactive-color de dentro
    // do bloco `layout` DE TOPO do
    // arquivo (rastreando profundidade de chaves por linha, com uma pilha de nomes de
    // bloco) — não mexe em focus-ring/border de dentro de um `window-rule` (ex.: a
    // regra do "DesktopOperator" mais abaixo no config.kdl, que zera o border só
    // daquela janela). Mapeamento combinado com o usuário: border.active-color =
    // Theme.red (acento forte), border.inactive-color = surface1; focus-ring fica
    // sutil e uniforme (surface1 nos dois estados, active e inactive) — o oposto do
    // mapeamento antigo (onde focus-ring e border usavam exatamente a mesma cor).
    function niriLayoutPatchPy() {
        const t = Theme
        const focusActive = hx(t.surface1)
        const focusInactive = hx(t.surface1)
        const borderActive = hx(t.red)
        const borderInactive = hx(t.surface1)
        return "import re, sys\n"
            + "path = sys.argv[1]\n"
            + "with open(path, encoding='utf-8') as f:\n"
            + "    lines = f.readlines()\n"
            + "FOCUS_ACTIVE = '" + focusActive + "'\n"
            + "FOCUS_INACTIVE = '" + focusInactive + "'\n"
            + "BORDER_ACTIVE = '" + borderActive + "'\n"
            + "BORDER_INACTIVE = '" + borderInactive + "'\n"
            + "color_re = re.compile(r'^(\\s*)(active-color|inactive-color)\\s+\"[^\"]*\"(.*)$')\n"
            + "open_re = re.compile(r'^\\s*([A-Za-z][\\w-]*)\\s*\\{')\n"
            + "stack = []\n"
            + "out = []\n"
            + "for line in lines:\n"
            + "    s = line.rstrip('\\n')\n"
            + "    opens = s.count('{')\n"
            + "    closes = s.count('}')\n"
            + "    in_target = len(stack) == 2 and stack[0] == 'layout' and stack[1] in ('focus-ring', 'border')\n"
            + "    m = color_re.match(s)\n"
            + "    if in_target and m:\n"
            + "        indent, prop, rest = m.groups()\n"
            + "        if stack[1] == 'focus-ring':\n"
            + "            newhex = FOCUS_ACTIVE if prop == 'active-color' else FOCUS_INACTIVE\n"
            + "        else:\n"
            + "            newhex = BORDER_ACTIVE if prop == 'active-color' else BORDER_INACTIVE\n"
            + "        s = f'{indent}{prop} \"{newhex}\"{rest}'\n"
            + "    if opens > closes:\n"
    ...
    function exportAll() {
        ...
        // config.kdl é do USUÁRIO (e agora também do NiriMod) — não é um alvo dedicado
        // nosso como os de cima, então em vez de sobrescrever o arquivo inteiro, faz
        // backup (mesmo padrão .bak-$ts) e reescreve só as 4 linhas de cor do bloco
        // `layout` via niriLayoutPatchPy(). Sem python3 instalado, pula silenciosamente
        // (o resto do export continua valendo).
        const configKdl = HOME + "/.config/niri/config.kdl"
        const patchB64 = Qt.btoa(niriLayoutPatchPy())
        script += "if [ -f '" + configKdl + "' ]; then\n"
            + "  rm -f '" + configKdl + "'.bak-* 2>/dev/null || true\n"
            + "  cp -f '" + configKdl + "' '" + configKdl + ".bak-'$ts\n"
            + "  if command -v python3 >/dev/null; then\n"
            + "    pyf=$(mktemp)\n"
            + "    printf %s '" + patchB64 + "' | base64 -d > \"$pyf\"\n"
            + "    python3 \"$pyf\" '" + configKdl + "'\n"
            + "    rm -f \"$pyf\"\n"
            + "  fi\n"
            + "fi\n"
