...
//              as linhas de cor do focus-ring/border E o `screenshot-path` do
//              config.kdl devem ficar comentadas p/ não competirem com este arquivo —
//              screenshot-path vem de Config.screenshotDir, editável na janela de
//              configurações em Gravação)
...
Singleton {
    ...
    // ── niri (devil-shell/theme.kdl) — cores do focus-ring/border + screenshot-path ──
    // O niri faz merge de blocos `layout` duplicados entre o config.kdl e os includes.
    // screenshot-path é um nó único (não um bloco); por isso a linha equivalente no
    // config.kdl do usuário deve ficar COMENTADA, senão os dois competem.
