...
fontes, tempos, ângulos dos cristais, áudio, gravação, clima) e os nomes **semânticos** de cor
(`ball`, `crystal`, `accent`…). Cada property segue o padrão `Settings.get("nome", <padrão>)`: o literal
...
- **Auto-descoberta só na RAIZ; subpastas pedem `import "root:/<pasta>"`** (ver "Estrutura de pastas"
  acima). Na raiz, um arquivo vira componente pelo nome (`Crystal { }`) e `pragma Singleton` + `Singleton { }`
